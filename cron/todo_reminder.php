                         ...
                         // the order could be random).
    {
      ...
      if (count($today)>0)
      {
        ...
        for ($i=0, $_i=count($today);$i<$_i;$i++)
        {
          ...
          {
            $body.="\n    ".text("owner").": ".$today[$i]["owner_lastname"].", ".$today[$i]["owner_firstname"]." <".$today[$i]["owner_email"].">";
          }
          // always show assigned_to. (bugs assigned to ourselve don't show up as a byyou, only as a toyou)
          else if ($who=="byyou")
          {
            $body.="\n    ".text("assigned_to").": ".$today[$i]["assigned_lastname"].", ".$today[$i]["assigned_firstname"]." <".$today[$i]["assigned_email"].">";
          }
          $body.="\n\n    ".str_replace("\n","\n    ",$today[$i]["description"])."\n\n";
        }
        $body.="\n";
      ...
      if (count($late)>0)
      {
        ...
        for ($i=0, $_i=count($late);$i<$_i;$i++)
        {
          ...
          {
            $body.="\n    ".text("owner").": ".$late[$i]["owner_lastname"].", ".$late[$i]["owner_firstname"]." <".$late[$i]["owner_email"].">";
          }
          // always show assigned_to. (bugs assigned to ourselve don't show up as a byyou, only as a toyou)
          else if ($who=="byyou")
          {
            $body.="\n    ".text("assigned_to").": ".$late[$i]["assigned_lastname"].", ".$late[$i]["assigned_firstname"]." <".$late[$i]["assigned_email"].">";
          }
          $body.="\n\n    ".str_replace("\n", "\n    ",$late[$i]["description"])."\n\n";
        }
      }
