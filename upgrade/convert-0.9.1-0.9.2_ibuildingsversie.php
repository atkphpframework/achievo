...
if(!empty($convert)&&$convert==1)
{
  ...
  $sql = "UPDATE accessright SET node = CONCAT('calendar.', node) WHERE node IN ('schedule', 'holidays', 'schedule_types', 'schedule_notes', 'schedule_attendees')";
  $res = $g_db->query($sql);
  handleError($sql);
  ...
  $res = $g_db->query($sql);
  handleError($sql);
  $sql = "UPDATE accessright SET node = CONCAT('mod_offerte.', node) WHERE node IN ('offerte')";
  $res = $g_db->query($sql);
  handleError($sql);
