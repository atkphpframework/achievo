...
class RadarPlot
{
    ...
    {
        JpGraphError::RaiseL($error);
    }

    function setCSIMTargets($csim, $alts)
    {
        $this->csim = $csim;
        $this->alts = $alts;
    }

    function calcCSIMAreas($pos, &$scale)
    {
        $pnts = array();
        ...
        if ($this->mark->show) {
            for ($i = 0; $i < $nbrpnts; ++$i) {
                $this->csimAreas[] = $this->AddCSIMCircle($pnts[$i * 2], $pnts[$i * 2 + 1], 6, $this->csim[$i]);
            }
        }
    }
    ...
    {
        $output = "";
        foreach ($this->csimAreas as $area) {
            $output.=$area . chr(13);
        }
        return $output;
    ...
    function AddCSIMCircle($x, $y, $r, $target)
    {
        $x = round($x);
        $y = round($y);
        $r = round($r);
        $csimareas = "";
        if (!empty($target)) {
            $csimareas .= "<area shape=\"circle\" coords=\"$x,$y,$r\" href=\"" . htmlentities($target) . "\"";

            if (!empty($this->csimwintarget)) {
                $csimareas .= " target=\"" . $this->csimwintarget . "\" ";
            }

            if (!empty($this->csimalt)) {
                $tmp = sprintf($this->csimalt, $this->yvalue, $this->xvalue);
                $csimareas .= " title=\"$tmp\" alt=\"$tmp\" ";
            }
            $csimareas .= " />\n";
        }
        return $csimareas;
...
class RadarGraph extends Graph
{
    ...
    function GetCSIMareas()
    {
        ...
        $n = count($this->plots);
        for ($i = 0; $i < $n; ++$i) {
            $this->plots[$i]->calcCSIMAreas($this->posy, $this->yscale);
