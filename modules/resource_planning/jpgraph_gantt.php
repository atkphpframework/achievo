...
//===================================================
class Progress {
    var $iProgress=-1, $iColor="seagreen", $iPattern=GANTT_SOLID;
...
//===================================================
// CLASS Planned
//Ludo M. Beumer (ludo@ibuildings.nl)
// Holds parameters for the planned indicator
// displayed within a bar
//===================================================
class Planned {
    var $iProgress=-1, $iPlanned=-1, $iColor="blue", $iPattern=GANTT_SOLID;
    var $iDensity=98, $iHeight=0.65;

    function Set($aProg, $aPlan) {
	if( $aProg < 0.0 || $aProg > 1.0 )
	    JpGraphError::Raise("<b>JpGraph Error:</b> Progress value must in range [0, 1]");
	$this->iProgress = $aProg;
	if( $aPlan < 0.0 || $aPlan > 1.0 )
	    JpGraphError::Raise("<b>JpGraph Error:</b> Planned value must in range [0, 1]");
	if(($aPlan + $aProg)>1.0)
	    JpGraphError::Raise("<b>JpGraph Error:</b> Progress and planned value together must below 1");
	$this->iPlanned = $aPlan;
    }
...
// PUBLIC METHODS
	...
	if( $this->planned->iProgress > 0 ) {
	    $prog = $factory->Create($this->planned->iPattern,$this->planned->iColor);
	    $prog->SetDensity($this->planned->iDensity);
	    $barheight = ($yb-$yt+1);
	    if( $this->iShadow )
		$barheight -= $this->iShadowWidth;
	    $progressheight = floor($barheight*$this->planned->iHeight);
	    $marg = ceil(($barheight-$progressheight)/2);
	    $pos=new Rectangle($xt+($xb-$xt+1)*$this->planned->iProgress,
	    $yt + $marg,
	    ($xb-$xt+1)*$this->planned->iPlanned,
	    $barheight-2*$marg);
	    $prog->SetPos($pos);
	    $prog->Stroke($aImg);
	}
